...
| REV-24b | BLOCKER doctrinal/metaontológico | OPEN | **Membership Completeness (MC): la clausura de $\Lambda_*$ puede omitir modos reales de co-pertenencia.** | Intersección REV-07/REV-24. | MC se formula directamente sobre `CoReal`; $\mathrm{RC}_{\Lambda}+\mathrm{CD}_i\Rightarrow\mathrm{MC}_i(S_i^*;q)$. Además FID + LA ⇒ RC, concentrando la carga profunda en si toda co-realidad admite descomposición integrativa finita. `CoReal` e `IntRel` deben caracterizarse independientemente de $\Lambda_*$. |
