...
| REV-07b | BLOCKER ontológico | PARTIAL | **GenEvent debe representar sound/complete la producción ontológica y conservar su footprint.** | $\operatorname{OntProd}_i$ / $\operatorname{GenEvent}_i$ / $\operatorname{GenFoot}_i$. | GenSound/GenComplete separan target e implementación; RootClosed usa OntProd y exige GenFoot⊆Seed para no dejar fuera el propio evento productor. $\Gamma_i$ añade el footprint completo. GenStep binario sigue siendo solo proyección. Falta caracterizar OntProd y los modos concretos. |
