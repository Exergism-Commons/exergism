...
| REV-24 | BLOCKER doctrinal | OPEN | **$S_i$ y $R_i$ tienen tipos distintos; la exhaustividad semántica no produce por sí sola un alcance ontológico total.** | Conclusión del teorema directo / `WitnessedR` / `ExistsR`. | Cerrar solo demostrando un puente no circular que produzca algún $R_i$ con $\operatorname{Presents}_i(S_i,R_i)\land\operatorname{OntTotal}_i(R_i)$. El trabajo se descompone en REV-24a/OA, REV-24b/MC y REV-24c/RA. `Presents` no puede contener `OntTotal` por definición. |
| REV-24a | BLOCKER doctrinal | PARTIAL | **Ontological Anchoring (OA): una estructura semántica no adquiere realidad por ser consistente o máxima.** | Denotación de $T_S,\Phi_S$. | Para $S_i^*$ el carrier $T_i=[q]_{\sim}$ está construido a partir de tokens actuales y enlaces declarados ontológicos, por lo que el anclaje de tokens queda condicionalmente descargado si REV-07 es correcto. Sigue abierto el anclaje/fidelidad de $\Phi_i^{\mathrm{all}}$ y la semántica de `Actual_i`, en coordinación con REV-25/24c. |
| REV-24b | BLOCKER doctrinal/metaontológico | OPEN | **Membership Completeness (MC): la clausura de $\Lambda_*$ puede omitir modos reales de co-pertenencia.** | Intersección REV-07/REV-24. | Se reduce a justificar $\mathrm{RS}_{\Lambda}$ y, sobre todo, $\mathrm{RC}_{\Lambda}:\operatorname{CoReal}(x,q)\Rightarrow x\in[q]_{\sim}$ con `CoReal` caracterizado independientemente. Bajo RS+RC y $T_i=[q]_{\sim}$ se deriva condicionalmente $\mathrm{MC}_i(S_i^*)$. Examinar explícitamente holismo global, dependencias de límite e infinitarias; explicar además cómo la extensión determina un alcance sin reificarlo por fiat. |
