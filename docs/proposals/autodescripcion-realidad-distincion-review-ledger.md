...
| REV-22 | BLOCKER fundacional | PARTIAL | **La smallness necesaria depende de la clausura de régimen elegida.** | Tamaño del carrier/poset. | Ruta finita: PON hace set-sized $[q]_{\sim}$. Ruta generalizada: basta `CSet` o una implementación `TransClSmall`; con `SigSmall_i` + `ActualSep_i` se construye un máximo semántico sobre ese carrier. La ruta directa evita Zorn; Zorn sigue requiriendo poset set-sized si se usa. |
...
| REV-24b | BLOCKER doctrinal/metaontológico | OPEN | **Membership Completeness (MC): una clausura de régimen puede omitir modos reales de co-pertenencia.** | Intersección REV-07/REV-24. | Ruta A: $RC_\Lambda+CD_i\Rightarrow MC_i(S_i^*;q)$ y FID+LA⇒RC, con WA en cada testigo. Contraejemplos abstractos (conectado/no path-connected) e infinitary dependence demuestran que FID no es una verdad lógica. Ruta B: clausura integrativa generalizada $\mathcal C_*$ con CS/CC/CSet/CProcStable; una implementación transfinita requiere `TransClSmall`. FID queda como condición suficiente de la ruta finita, no condición necesaria de `ExistsR`. |
...
| 2026-09-22 | Generalized semantic theorem | Formalizada la ruta $\mathcal C_*$: CSet + CProcStable + SigSmall + ActualSep + StructAdm ⇒ $SemTotal_{i,\mathcal C}(S_i^{\mathcal C})$. PON queda como blocker de la instanciación finita, no requisito universal del teorema semántico. | REV-22/23/24b + derivaciones. | 67e0291, 7c1513c |
