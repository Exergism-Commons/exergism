...
| REV-07a | BLOCKER ontológico | OPEN | **OriginUnity debe distinguir una configuración real de una mera suma de raíces.** | $\operatorname{OriginConfig}_i$ / $\operatorname{Bind}_i$. | Justificar qué relaciones concretas cuentan como Bind/integración sin usar $R_i$ ni Generated$^*$. OU2 ya demuestra que un agregado sin Bind cruzando alguna partición no puede ser OntOrigin. |
| REV-07b | BLOCKER ontológico | PARTIAL | **GenEvent debe ser una representación sound y complete de producción ontológica real.** | $\operatorname{OntProd}_i$ / $\operatorname{GenEvent}_i(e_i,A_i,b_i)$. | Se separan GenSound y GenComplete. RootClosed se evalúa contra OntProd para no declarar ausencia de entradas por un inventario incompleto. El contraejemplo $\{a,b\}\to c$ demuestra además que GenStep binario no preserva closure. Falta caracterizar OntProd y justificar por modo causal/constitutivo/grounding/continuidad. |
