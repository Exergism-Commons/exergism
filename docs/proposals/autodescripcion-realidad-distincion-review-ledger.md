...
| REV-24b | BLOCKER doctrinal/metaontológico | OPEN | **Membership Completeness (MC): la presentación/reconstrucción puede omitir contenido del mismo índice.** | Cobertura de $\operatorname{Generated}^{*}_i(\mathcal O_i,-)$. | EXT-01 elimina el problema de cruzar índices pero no garantiza conectividad interna. Sigue abierto demostrar RC/CC: todo $Real_i(x)$ debe aparecer en la reconstrucción elegida. FID es solo una vía suficiente para Ruta A y puede fallar en cierres globales/no path-connected. |
