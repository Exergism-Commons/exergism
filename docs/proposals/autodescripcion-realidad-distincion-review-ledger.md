...
| REV-07 | BLOCKER | PARTIAL | **La identidad de régimen debe fundarse en origen + clausura ontológica, no en K3 ni en conectividad elegida por conveniencia.** | $\operatorname{OntOrigin}_i$, $\operatorname{GenStep}_i$, $\operatorname{Generated}^{*}_i$. | Justificar independientemente la base/origen ontológico, las reglas generativas y su clausura. $\Lambda_*$ y $\mathcal C_*$ pasan a ser reconstrucciones candidatas que deben ser sound/complete respecto de la genealogía. Scope realization se mueve aquí desde REV-24d. |
...
| REV-24a | BLOCKER doctrinal | PARTIAL | **Ontological Anchoring (OA): el carrier semántico no puede sobreincluir contenido ajeno a la clausura genealógica.** | Denotación de $T_S$ / soundness respecto de $\operatorname{Generated}^{*}_i$. | Reformulado como $\forall a\in T_S\exists x[Generated_i^*(\mathcal O_i,x)\land Den_i(a,x)]$. RS/CS se reinterpretan como soundness de las reconstrucciones $\Lambda_*$/$\mathcal C_*$ respecto de la genealogía. |
| REV-24b | BLOCKER doctrinal/metaontológico | OPEN | **Membership Completeness (MC): la presentación puede omitir contenido generado por el origen ontológico.** | Cobertura de $\operatorname{Generated}^{*}_i(\mathcal O_i,-)$. | Reformulado como $Generated_i^*(\mathcal O_i,x)\Rightarrow\exists a\in T_S Den_i(a,x)$. RC/CC se reinterpretan como completeness de la reconstrucción frente a la genealogía; FID sigue siendo solo una vía suficiente para Ruta A. |
