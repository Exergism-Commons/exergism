...
| REV-07 | BLOCKER | PARTIAL | **La directedness interna K3_i se deriva de StructAdm; solo permanece abierta la individuación filosófica del régimen mediante $\Lambda_*$.** | Criterio de pertenencia a $i$. | Para dominios del mismo régimen, la unión semántica positiva actual es admisible y proporciona cota común, por lo que K3_i queda demostrado. EEA también se deriva para footprints actuales. Falta justificar qué relaciones pertenecen a $\Lambda_*$, su invariancia representacional y que la taxonomía no colapse ni fragmente indebidamente regímenes. |
...
| REV-22 | BLOCKER fundacional | PARTIAL | **La aplicabilidad set-sized de Zorn se deriva condicionalmente de PON + firma set-sized/finitaria, pero PON aún no está justificada.** | Tamaño del poset / fundamentos del teorema. | PON hace set-sized cada componente $[q]_{\sim}$; con firma set-sized/finitaria, el universo de hechos y la colección de fragmentos StructAdm son sets. Así la ruta A de REV-22 queda demostrada condicionalmente. La ruta directa a Exists-R además evita Zorn por completo. Cerrar cuando REV-23 quede justificada o se adopte otra base fundacional. |
| REV-23 | BLOCKER fundacional/metaontológico | OPEN | **PON no está justificada: un token podría tener proper-class many vecinos ontológicos inmediatos bajo $\bowtie$.** | Ruta directa a Exists-R; smallness del régimen. | Justificar independientemente que $N(q)=\{r\mid q\bowtie r\}$ es set-sized para todo token actual, o sustituir PON por una condición más débil que siga haciendo set-sized cada componente finitamente conectado. No vale definir $\Lambda_*$ o «régimen» de forma que smallness resulte por estipulación. |
| REV-18 | BLOCKER | RESOLVED | **El puente mínimo entre contenido emergente y dominio procesual queda tipado mediante fragmentos semánticos positivos actuales.** | Interpretación emergentista de $\mathfrak K$. | Se adopta $X=(T_X,\Phi_X)$ con $\operatorname{Adm}_i:=\operatorname{StructAdm}_i$; incidencia es pertenencia al carrier semántico y `EClosed_i` es una propiedad del contenido. El dominio no se reifica como entidad adicional. Reabrir si se exige una noción de dominio más fuerte que fragmento positivo actual well-formed. |
...
- Con firma semántica set-sized/finitaria y StructAdm se construye directamente $R_i^*=([q]_{\sim},\Phi_i^{\mathrm{all}})$.
...
| 2026-09-22 | REV-20/REV-22 | PON implica PSB/K1 y hace set-sized el régimen/poset bajo firma finitaria set-sized; ambas deudas dejan de ser blockers independientes de la ruta directa. | §4.0 + ledger. | af794e7, ba9af92 |
...
Estado sustantivo actual: REV-03 y REV-07 permanecen PARTIAL; REV-20 y REV-22 permanecen PARTIAL como rutas subordinadas/alternativas; REV-15 y REV-23 permanecen OPEN. Para Exists-R por la ruta directa, los blockers sustantivos son REV-07 (identidad de régimen) y REV-23 (PON), además de la suficiencia doctrinal de REV-03 si se quiere que `EClosed` tenga contenido emergentista fuerte. REV-01, REV-02, REV-04, REV-08, REV-10, REV-14, REV-19 y REV-21 están RESOLVED en su alcance explícitamente acotado. El objetivo mínimo es Exists-R: demostrar al menos un máximo E-closed $R_i$; One-R global y $R_{\mathrm{abs}}$ son opcionales respecto de ese objetivo.
