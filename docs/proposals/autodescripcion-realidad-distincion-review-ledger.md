...
| REV-07d | BLOCKER doctrinal | OPEN | **RootClosed permite ciclos internos pero puede excluir genealogías sin fuente.** | OU3 / concepto de origen. | Decidir si un regreso generativo esencialmente sin source/fixed seed falsifica ExistsR o si debe admitirse una noción de origen-límite/fixed-point. No introducirla solo para garantizar existencia. |
| REV-07e | BLOCKER doctrinal/metaontológico | OPEN | **Problema de convergencia:** interacción real entre linajes aparentemente independientes exige explicar por qué comparten origen. | CGP / relación entre EXT-01 y origen común. | Justificar `Integrable^M(C_a,C_b) => CommonGround^M(C_a,C_b)` sin definir CommonGround como «mismo índice». Si CGP falla, revisar la equivalencia entre compatibilidad ontológica y genealogía común. |
