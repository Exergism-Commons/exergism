...
| REV-07 | BLOCKER | PARTIAL | **La identidad de régimen se funda en origen + clausura ontológica; el índice es la frontera de incompatibilidad.** | $\operatorname{OntOrigin}_i$, $\operatorname{GenStep}_i$, $\operatorname{Generated}^{*}_i$. | Justificar independientemente origen, reglas generativas y clausura. EXT-01 + EdgeTyped + seed tipado derivan soundness de la Ruta A: un camino real no cruza índices. La deuda fuerte de reconstrucción queda en completeness (RC/CC) y scope realization. |
...
| REV-24a | BLOCKER doctrinal | PARTIAL | **Ontological Anchoring (OA): el carrier semántico no puede sobreincluir contenido ajeno a la clausura genealógica.** | Denotación de $T_S$ / soundness. | En Ruta A, $Real_i(q)$ + EdgeTyped + EXT-01 derivan $x\in[q]_{\sim}\Rightarrow Real_i(x)$, por lo que la sobreinclusión entre índices queda cerrada condicionalmente. Sigue pendiente el anclaje semántico/denotacional completo y la Ruta B. |
