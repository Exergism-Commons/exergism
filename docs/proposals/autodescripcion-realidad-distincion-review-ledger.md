...
| REV-26b | BLOCKER lógico/fundacional | OPEN | **APC_Real — comprensión plural absoluta relevante.** | Realización plural de `Scope(Real)`. | Justificar la instancia `(∃x Real(x)) ⇒ ∃rr_abs∀x[x≺rr_abs ↔ Real(x)]` bajo el alcance absoluto pretendido. Critical plural logic muestra que comprensión plural no debe darse por gratuita en dominios extensionally indefinite. |
