...
| REV-06 | MAJOR | RESOLVED | **Muchos [I] son consecuencias definicionales o estructurales.** | [I1], [I2], [I2a], REC, [I10a], [I11a], etc. | REC queda clasificado explícitamente como consecuencia estructural/analítica condicionada a `OntTotal`: una totalidad real no puede tener un certificador real ontológicamente exterior. No se usa REC para demostrar `OntTotal` ni `ExistsR`; eliminar retórica probatoria donde no corresponda. |
| REV-07 | BLOCKER | PARTIAL | **La directedness interna K3_i se deriva de StructAdm; permanece abierta la adecuación ontológica de la individuación mediante $\Lambda_*$.** | Criterio de pertenencia a $i$ / REV-24b. | Además de justificar roles e invariancia, deben distinguirse $\mathrm{RS}_{\Lambda}$ (soundness: la conectividad no fusiona realidades distintas) y $\mathrm{RC}_{\Lambda}$ (completeness: toda co-pertenencia ontológica queda en una cadena finita de enlaces). $\mathrm{RC}_{\Lambda}$ es la deuda compartida con REV-24b; no vale definir `CoReal` por la propia conectividad. |
...
| REV-24 | BLOCKER doctrinal | OPEN | **$S_i$ y $R_i$ tienen tipos distintos; la exhaustividad semántica no produce por sí sola un alcance ontológico total.** | Conclusión del teorema directo / `WitnessedR` / `ExistsR`. | Cerrar solo demostrando un puente no circular que produzca algún $R_i$ con $\operatorname{Presents}_i(S_i,R_i)\land\operatorname{OntTotal}_i(R_i)$. El trabajo se descompone en REV-24a/OA, REV-24b/MC, REV-24c/RA y REV-24d/SR. `Presents` no puede contener `OntTotal` por definición y SR no puede esconder una comprensión/reificación de scopes. |
| REV-24a | BLOCKER doctrinal | PARTIAL | **Ontological Anchoring (OA): el carrier semántico no puede sobreincluir contenido ajeno al alcance ontológico pretendido.** | Denotación de $T_S$ / soundness de $\Lambda_*$. | OA se formula directamente sobre `CoReal`. Para $S_i^*$ se demuestra condicionalmente $\mathrm{RS}_{\Lambda}+\mathrm{CD}_i\Rightarrow\mathrm{OA}_i(S_i^*;q)$. El anclaje/fidelidad de $\Phi_i^{\mathrm{all}}$ sigue abierto bajo REV-24c/REV-25. |
| REV-24b | BLOCKER doctrinal/metaontológico | OPEN | **Membership Completeness (MC): la clausura de $\Lambda_*$ puede omitir modos reales de co-pertenencia.** | Intersección REV-07/REV-24. | MC cuantifica directamente sobre `CoReal`; $\mathrm{RC}_{\Lambda}+\mathrm{CD}_i\Rightarrow\mathrm{MC}_i(S_i^*;q)$. FID + LA ⇒ RC, pero FID significa profundidad finita de testigos, no generación local finita. Gauge/QFT/superselección muestran que localidad/aditividad pueden fallar sin refutar FID. Todo testigo global/infinitario debe satisfacer `WA` independientemente; AQFT/cuasilocalidad queda como stress test de contenido de límite. `CoReal`, `IntRel` y `WA` no pueden definirse por la conectividad que intentan justificar. |
...
| REV-24d | BLOCKER doctrinal/fundacional | OPEN | **Scope Realization (SR): una extensión/predicado bien caracterizado no produce automáticamente un objeto del tipo alcance ontológico.** | Paso `CoReal(-,q)` → $R_i$. | Justificar $\mathrm{SR}_i(q):\exists R_i\forall x[\operatorname{Within}_i(x,R_i)\leftrightarrow(\operatorname{Actual}(x)\land\operatorname{CoReal}(x,q))]$ sin identificar $R_i$ con un set ni introducir una comprensión de scopes tácita. Si se usa lógica plural/segundo orden o comprensión de alcances, declarar el compromiso. |
...
- El objetivo fuerte sigue siendo refutar $\operatorname{NoR}$, donde $\operatorname{ExistsR}:=\exists i\exists R_i\,\operatorname{OntTotal}_i(R_i)$. Un máximo E-closed solo produce $\operatorname{SemTotal}_i(S_i)$; la ruta constructiva debe llegar a $\operatorname{WitnessedR}$ justificando $\operatorname{Presents}_i(S_i,R_i)\land\operatorname{OntTotal}_i(R_i)$ mediante REV-24a/b/c.
...
| 2026-09-22 | REV-24 decomposition | `ExistsR` se retipa como existencia puramente ontológica; se introduce `WitnessedR` para la ruta semánticamente presentada. REV-24 se descompone en OA (anclaje), MC (completitud de pertenencia, acoplada a REV-07) y RA (adecuación representacional); `Presents_i(S_i,R_i)` reemplaza la antigua aplicación mal tipada `OntTotal_i(S_i)`. | §§1.1, 1.4, 3.2, 4.0, 7 + documento técnico. | b4ab4da, 27ffbcf |
