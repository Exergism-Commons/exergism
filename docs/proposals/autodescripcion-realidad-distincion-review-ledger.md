...
| REV-07a | BLOCKER ontológico | PARTIAL | **OriginUnity debe distinguir una base cofundamental de una suma o convergencia sin depender del candidato.** | $\operatorname{UnitFact}_i$ / $\operatorname{EssConDep}_i$ / $\operatorname{ConstitutiveBridge}_i$ / $\operatorname{UnitFoot}_i$. | La forma lógica ya está fijada: cada partición no trivial requiere un ConstitutiveBridge bilateral y footprint interno. OriginConstitutive queda SUPERSEDED; M1–M6 fijan los modelos de control, ECD1–ECD7 el contrato y MOD/RLC/HC/PC la taxonomía núcleo. Causalidad ordinaria, interacción, input/output y participación contingente quedan excluidos por defecto. Falta justificar independientemente qué esquemas de la taxonomía acepta la ontología y su cobertura. |
