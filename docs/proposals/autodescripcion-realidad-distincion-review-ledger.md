...
| REV-07b | BLOCKER ontológico | PARTIAL | **GenEvent debe representar sound/complete una OntProd objetiva y conservar su footprint.** | $\operatorname{OntProd}_i$ / ProdCoverage / GenAdequate / CAU-CON-GRD-PRC / $\operatorname{GenFoot}_i$. | OP1–OP8 caracterizan modos objetivos independientes; CAU/CON/GRD/PRC implican OntProd y GenEvent implementa esa taxonomía, por lo que GenSound queda derivado. ProdCoverage implica GenComplete y, con ello, GenAdequate; OntOrigin y Real↔Generated quedan ahora explícitamente guardados por GenAdequate. GF1–GF6 y P1–P8 bloquean soporte omitido/inflado y pérdida de hiperaridad. Falta justificar ProdCoverage en el contexto usado por ExistsR. |
...
| REV-07e | BLOCKER doctrinal/metaontológico | OPEN | **Convergencia no equivale a co-origen.** Un hecho común establece joint realizability y corrige una incompatibilidad provisional, pero no convierte al propio evento en fundamento común. | JointRealizable / CGP / EXT-01–02. | Mantener `Integrable^M => JointRealizable^M` separado de la tesis fuerte OPEN `JointRealizable^M => CommonGround^M`. Si CGP se justifica, la retroactividad es epistémica: la convergencia revela un fundamento ontológico independiente del evento convergente, sin exigir prioridad temporal. Si falla, una ontogénesis por convergencia requiere una extensión formal explícita de la teoría. |
