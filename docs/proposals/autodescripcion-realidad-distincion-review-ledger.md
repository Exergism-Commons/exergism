...
| REV-07 | BLOCKER | PARTIAL | **La identidad de régimen ya se separa de K3 y la familia pre-régimen de enlaces está restringida por criterios explícitos, pero aún falta justificar filosóficamente qué tipos de relación tienen fuerza integradora suficiente y por qué cada régimen satisface directedness interna.** | Criterio de pertenencia a $i$, admisibilidad de $\Lambda_*$ y $K3_i$. | Avance: $\Lambda_*$ debe ser actual, ontológica, token-specific, integradora, semánticamente local e invariante bajo representación; se excluyen semejanza, universal compartido, ley compartida, isomorfismo, co-representación y mera posibilidad causal. La clausura $\sim$ genera regímenes candidatos y `SameRegime` no implica K3. Falta justificar los roles integradores admitidos, resolver casos controvertidos de constitución/dependencia y derivar/justificar $K3_i$ mediante una premisa adicional no circular. |
