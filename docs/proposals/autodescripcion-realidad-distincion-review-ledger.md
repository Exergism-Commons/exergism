...
| REV-07 | BLOCKER | PARTIAL | **La realidad indexada se funda en una configuración-originaria unificada + least generative closure.** | $\operatorname{OntOrigin}_i$, $\operatorname{GenEvent}_i$, $\operatorname{Cl}^{G}_i$. | Ya se bloquean dos trivializaciones: una unión metalingüística de raíces independientes falla OriginUnity, y la generación multiantecedente no puede reducirse a GenStep binario. Quedan REV-07a–e, reconstruction completeness y scope realization. |
| REV-07a | BLOCKER ontológico | PARTIAL | **OriginUnity debe distinguir una base cofundamental de una suma o convergencia sin depender del candidato.** | $\operatorname{UnitFact}_i$ / $\operatorname{EssConDep}_i$ / $\operatorname{ConstitutiveBridge}_i$ / $\operatorname{UnitFoot}_i$. | La forma lógica ya está fijada: cada partición no trivial requiere un ConstitutiveBridge con dependencia constitutiva esencial cruzada hacia ambos lados y footprint interno. El antiguo OriginConstitutive binario queda SUPERSEDED. Los modelos átomo/ciclo/holismo pasan; suma independiente/convergencia tardía/grounding unilateral fallan como se pretende. Falta justificar una taxonomía independiente de modos ontológicos que instancien EssConDep con cobertura suficiente. |
