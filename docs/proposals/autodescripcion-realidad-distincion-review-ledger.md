...
| REV-07 | BLOCKER | PARTIAL | **La identidad de régimen ya se separa de K3, pero todavía hay que justificar qué relaciones ontológicas actuales constituyen esa identidad y por qué cada régimen satisface directedness interna.** | Criterio de pertenencia a $i$ y $K3_i$. | Avance: se define una relación de enlace actual $\bowtie$ a partir de una familia $\Lambda$ independiente de amalgamación; su clausura de equivalencia $\sim$ genera regímenes candidatos, y un contraejemplo demuestra $\operatorname{SameRegime}(X,Y)\not\Rightarrow K3_i(X,Y)$. Falta justificar ontológicamente $\Lambda$, demostrar invariancia representacional suficiente y derivar/justificar $K3_i$ mediante una premisa adicional no circular. |
...
| 2026-09-22 | FORM-01/02/03 | Documento vigente: 672 líneas, 0 pares de delimitadores inline por barra inversa-paréntesis, 0 líneas `# Ledger de revisión — autodescripción, realidad y distinción
