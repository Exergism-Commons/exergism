...
| REV-07 | BLOCKER | PARTIAL | **La identidad de régimen se separa de K3 y $K3_i$ ya se reduce a dos principios locales, pero todavía falta justificar filosóficamente $\Lambda_*$ y ontológicamente EEA/OAM.** | Pertenencia a $i$, admisibilidad de $\Lambda_*$ y directedness interna. | Avance: $\Lambda_*$ tiene seis constraints explícitos; `SameRegime` no implica K3; y se demuestra $\mathrm{EEA}+\mathrm{OAM}\Rightarrow K3_i$ para dominios no vacíos (más bottom si se admite vacío). Falta justificar los roles integradores de $\Lambda_*$, la estabilidad representacional, Edge Extension Admissibility y Overlap Amalgamation desde la ontología de dominios procesuales; estas obligaciones conectan directamente con REV-18/REV-19. |
...
| REV-20 | BLOCKER | OPEN | **Cofinalidad K1 de dominios E-closed no demostrada.** | Existencia de extensiones cerradas. | Cerrar solo mediante una justificación estructural/no enumerativa de que todo $X\in\mathfrak D_i^{\mathrm{proc}}$ admite algún $Y\in\mathfrak K_i$ con $X\preceq_i Y$. El Muro excluye usar muestreo interno como demostración de la cuantificación universal. La prueba no puede presuponer ya $R_i$, una unión global ni un operador ontológico de cierre por definición. |
