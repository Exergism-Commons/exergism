...
| REV-07 | BLOCKER | PARTIAL | **La realidad indexada se funda en una configuración-originaria unificada + least generative closure.** | $\operatorname{OntOrigin}_i$, $\operatorname{GenEvent}_i$, $\operatorname{Cl}^{G}_i$. | Ya se bloquean dos trivializaciones: una unión metalingüística de raíces independientes falla OriginUnity, y la generación multiantecedente no puede reducirse a GenStep binario. Quedan REV-07a–d, reconstruction completeness y scope realization. |
| REV-07a | BLOCKER ontológico | OPEN | **OriginUnity debe distinguir una configuración real de una mera suma de raíces sin depender del candidato ni reciclar convergencias tardías.** | $\operatorname{OriginConfig}_i$ / $\operatorname{UnitFact}_i$ / $\operatorname{OriginConstitutive}_i$ / $\operatorname{UnitFoot}_i$. | OriginUnity exige, para cada partición no trivial, un UnitFact independientemente origin-constitutive cuyo footprint completo sea interno al seed. UnitFact y OriginConstitutive no pueden mencionar el contenedor/Seed/OriginUnity ni el target; una interacción posterior no adquiere carácter originario por seed-stuffing retrospectivo. Falta justificar semánticamente qué relaciones concretas satisfacen UF1–UF5 y OC1–OC5. |
