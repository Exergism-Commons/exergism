...
| REV-07 | BLOCKER | PARTIAL | **K3/directedness debe justificarse dentro de cada régimen ontológico $i$ sin definir «mismo régimen» como «amalgamable».** La lectura global de K3 ya no forma parte del teorema emergentista. | Identidad del régimen $i$ y directedness interna $K3_i$. | Avance: el teorema fija $i$ antes de K1–K3 y solo concluye un máximo interno $R_i$, por lo que K3 ya no se usa como argumento a favor de One-R global. Falta un criterio independiente que determine cuándo dos dominios pertenecen al mismo $i$ y una justificación de $K3_i$ que no sea circular. |
