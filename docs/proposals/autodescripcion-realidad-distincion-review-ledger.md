...
| REV-26 | MAJOR metaontológico no bloqueante | OPEN | **One-$R$/Many-$R$ y comparación entre índices.** | Metalenguaje de índices. | No bloquea `ExistsR`. Investigar si existe un único índice o varios. Un origen común, token común o hecho transversal entre dos candidatos implica que no eran índices genuinamente distintos; Many-$R$ genuino es incompatible por tipado. |
| REV-26a | MAJOR lógico/metaontológico | DEFERRED | **AG — Absolute Generality.** | Metalenguaje global opcional. | Ya no es necesaria para `ExistsR`, cuya ontología es indexada. Reabrir solo si se pretende una cuantificación absoluta conjunta sobre todos los índices. |
...
| EXT-01 | DOCTRINAL | RESOLVED | **Incompatibilidad = índice.** Para índices ontológicos genuinos $i\neq j$ no hay tokens reales compartidos, hechos transversales, relaciones ontológicas ni origen común. | Many-$R$ / Cluster-$R$ / Muro. | Regla tipada: todo `RealFact_i` tiene solo relata `Real_i`; cualquier `CrossRealFact(i,j)` o `CommonOntOrigin(i,j)` fuerza $i=j$. Cluster-$R$ solo clasifica candidatos pre-indexados; genuine Many-$R$ no puede colapsar posteriormente. |
| REV-26e | BLOCKER doctrinal | RESOLVED | **Target doctrinal decidido.** | Qué significa $R$. | `ExistsR` significa que existe al menos un índice $i$ cuya realidad es la clausura de una base/origen ontológico común. One-$R$/Many-$R$ son metatesis posteriores; no se exige una clausura absoluta transversal. |
...
- One-$R$/Many-$R$ son metatesis sobre la cardinalidad de índices genuinos y pertenecen a REV-26.
...
Estado sustantivo actual: REV-03 y REV-07 permanecen PARTIAL; REV-20 y REV-22 permanecen PARTIAL como rutas subordinadas/alternativas; REV-15, REV-24 y REV-25 permanecen OPEN; REV-23 permanece OPEN solo para la ruta finita. `ExistsR` es ya el target indexado de REV-07; REV-24 apunta a `WitnessedR`; REV-26 queda como extensión metaontológica no bloqueante. EXT-01 fija que la incompatibilidad ontológica es la diferencia de índice.
