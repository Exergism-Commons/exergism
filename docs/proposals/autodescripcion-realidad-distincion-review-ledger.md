...
| REV-22 | BLOCKER fundacional | OPEN | **Aplicabilidad de Zorn al régimen real no establecida si $\mathfrak D_i^{\mathrm{proc}}$ es class-sized.** | Tamaño del poset / fundamentos del teorema. | Justificar una de tres rutas: (a) $\mathfrak D_i^{\mathrm{proc}}$ es set-sized; (b) existe un subposet set-sized cofinal $\mathfrak C_i$ que preserve K1/K2/K3 y cuya maximalidad domine todo el régimen; o (c) especificar una teoría de clases y principio de maximalidad suficientemente fuerte. No asumir smallness a partir de la existencia de $R_i$. |
...
| REV-20 | BLOCKER | PARTIAL | **K1 queda derivada salvo una condición puntual de smallness: PSB.** | Existencia de extensiones cerradas. | Con StructAdm, extensiones de eventos y uniones set-indexed preservan admisibilidad. PSB exige que para cada estado $s$, $\operatorname{Out}_i(s)$ sea set-sized; por Replacement + Union implica LSE/SO y la construcción por rondas produce una extensión E-closed de todo dominio set-sized. Cerrar REV-20 justificando PSB como propiedad estructural del régimen o derivándola de una hipótesis fundacional/local independiente. |
