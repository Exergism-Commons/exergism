...
| REV-24a | BLOCKER doctrinal | OPEN | **Ontological Anchoring (OA): una estructura semántica no adquiere realidad por ser consistente o máxima.** | Denotación de $T_S,\Phi_S$. | Definir y justificar una relación $\operatorname{Den}_i$ independiente tal que todo token/hecho usado por la presentación remita a contenido actual del régimen; OA no debe implicar completitud ni totalidad. |
| REV-24b | BLOCKER doctrinal/metaontológico | OPEN | **Membership Completeness (MC): el criterio independiente de co-régimen puede omitir modos reales de pertenencia.** | Intersección REV-07/REV-24. | Justificar $\forall x[(\operatorname{Actual}(x)\land\operatorname{Reg}_i(x))\Rightarrow\exists a\in T_S\,\operatorname{Den}_i(a,x)]$ con $\operatorname{Reg}_i$ fijado sin usar $S_i$, $R_i$, K3 ni totalización; explicar además cómo esa extensión determina un alcance ontológico sin reificarlo por fiat. |
