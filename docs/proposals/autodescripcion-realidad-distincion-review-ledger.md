...
| REV-24 | BLOCKER doctrinal local | OPEN | **$S_i$ no produce existencia ontológica; debe presentar correctamente una realidad genealógica ya justificada por REV-07.** | `WitnessedRegR` / $\operatorname{Presents}_i$. | Dado $\operatorname{GeneTotal}_i(\mathcal O_i,R_i)$, cerrar demostrando OA + MC + RA ⇒ $\operatorname{Presents}_i(S_i,R_i)$. REV-24 fortalece `ExistsRegR` a `WitnessedRegR`; no crea $R_i$ ni decide `ExistsR`. |
...
| REV-26 | BLOCKER doctrinal/metaontológico | OPEN | **Existencia del $R$ absoluto genealógico:** formar el scope extensional de todo lo real no demuestra una genealogía común. | $\operatorname{ExistsR}=\operatorname{ExistsAbsR}$. | El target ya está fijado como $\operatorname{AbsGeneTotal}(\mathcal O_{abs},R_{abs})$. Cerrar requiere AG para la lectura absoluta, una Common-Origin Thesis sound/exhaustive y una realización de scope adecuada. `ExistsRegR` no basta. |
...
| REV-26b | BLOCKER lógico/fundacional | PARTIAL | **APC_Real / scope realization es auxiliar, no la tesis ontológica de $R$.** | Realización plural de $\mathcal R_{ext}$ o de una clausura genealógica ya fijada. | PluralComp puede realizar el scope, pero $ExistsExtScope\not\Rightarrow ExistsR$. Sigue pendiente justificar comprensión plural absoluta si se usa; el contenido sustantivo pasa a REV-26c. |
| REV-26c | BLOCKER metaontológico | OPEN | **CO_abs / genealogical globalization:** todo lo real debe compartir una base ontológica común cuya clausura sea sound y exhaustiva. | $\mathcal O_{abs}$ / $\operatorname{Generated}^{*}_{abs}$. | Justificar $\exists\mathcal O_{abs}[OntOrigin_{abs}(\mathcal O_{abs})\land\forall x(Real(x)\leftrightarrow Generated^*_{abs}(\mathcal O_{abs},x))]$ con reglas generativas independientes. Desde Indexed-R, justificar una base superior común preservando genealogía, no mera inclusión extensional. |
| REV-26d | BLOCKER lógico/meta-semántico | OPEN | **No-$R$ no tiene la misma forma bajo absolutismo y generality relativism.** | Negación de la genealogía absoluta / expansionismo. | Bajo AG, $NoR_{AG}$ niega la existencia de una base/origen cuya clausura genere todo lo real y sea realizable como scope. Si AG se rechaza, mantener `NoAbsFinality` como metaschema: toda genealogía candidata admite expansión legítima no absorbida. |
| REV-26e | BLOCKER doctrinal | RESOLVED | **Target doctrinal decidido.** | Qué significa $R$. | $R$ no es el mero $\mathcal R_{ext}=Scope(Real)$: es el alcance de la clausura de una base/origen ontológico común. La unidad genealógica no se añade ad hoc; constituye el criterio doctrinal elegido de Realidad. |
...
Estado sustantivo actual: REV-03 y REV-07 permanecen PARTIAL; REV-20 y REV-22 permanecen PARTIAL como rutas subordinadas/alternativas; REV-15, REV-24, REV-25 y REV-26 permanecen OPEN; REV-23 permanece OPEN solo para la ruta finita; REV-26e está RESOLVED doctrinalmente. La construcción semántica demuestra condicionalmente $\operatorname{SemTotal}$. REV-07 concentra la existencia genealógica local; REV-24 apunta solo a `WitnessedRegR` mediante `Presents`; REV-26c concentra la Common-Origin Thesis absoluta. $\mathcal R_{ext}=Scope(Real)$ no se identifica con $R$.
