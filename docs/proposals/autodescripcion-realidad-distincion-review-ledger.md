...
| REV-24b | BLOCKER doctrinal/metaontológico | OPEN | **Membership Completeness (MC): la clausura de $\Lambda_*$ puede omitir modos reales de co-pertenencia.** | Intersección REV-07/REV-24. | MC se formula directamente sobre `CoReal`: $\mathrm{MC}_i(S;q)$. Para $S_i^*$, $\mathrm{RC}_{\Lambda}+\mathrm{CD}_i\Rightarrow\mathrm{MC}_i(S_i^*;q)$; RS no es necesaria para cobertura y queda como condición anti-sobreinclusión. `CoReal` debe caracterizarse independientemente de $\Lambda_*$. Examinar holismo global, dependencias de límite e infinitarias; explicar además cómo la extensión determina un alcance sin reificarlo por fiat. |
