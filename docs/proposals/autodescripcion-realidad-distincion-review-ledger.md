...
| REV-07e | BLOCKER doctrinal/metaontológico | OPEN | **Integración productiva o puramente relacional no equivale a co-origen.** | JointRealizable / RelIntegrable / CGP / context formation / EXT-01–02. | Tanto Integrable como RelIntegrable solo implican JointRealizable. Una cadena coherente $a-\rho-d\to c-\sigma-b$ puede probar un mismo marco realizado sin probar OntProd entre extremos, causalidad ni CommonGround; realizabilidad pairwise no se toma como transitiva. Si existen genealogías independientes unidas por PureOntRel y sin common ground, habrá que decidir entre CGP, ensamblaje de genealogías, ontogénesis nueva o mantener JointRealizable estrictamente más débil que pertenecer al mismo $R_i$. |
...
| REV-24 | BLOCKER doctrinal local | OPEN | **$S_i$ no produce existencia ontológica; debe presentar correctamente una realidad genealógica ya justificada por REV-07.** | `WitnessedR` / $\operatorname{Presents}_i$. | Dado $\operatorname{GeneTotal}_i(\mathcal O_i,R_i)$, cerrar demostrando OA + MC + RA ⇒ $\operatorname{Presents}_i(S_i,R_i)$. REV-24 fortalece `ExistsR` a `WitnessedR`; no crea $R_i$. |
...
| REV-26 | MAJOR metaontológico no bloqueante | OPEN | **One-$R$/Many-$R$ como metatesis sobre realizaciones tipadas.** | Metalenguaje de contextos. | No bloquea `ExistsR`. Los índices son metavariables, no miembros de un dominio $I$. One/Many-R se expresan mediante meta-cuantificación sobre realizaciones admisibles y los juicios $i\simeq_{idx}j$ / $i\#j$. |
...
| EXT-01 | DOCTRINAL | RESOLVED | **Incompatibilidad = diferencia de contexto.** | Many-$R$ / Cluster-$R$ / Muro. | Para realizaciones genuinas se usa el juicio metateórico $i\#j$, no un predicado ontológico. Términos de tipo $i$ no pueden aparecer en fórmulas de $j$. Un hecho/origen común solo puede diagnosticarse antes de tipar definitivamente y fuerza $i\simeq_{idx}j$. |
