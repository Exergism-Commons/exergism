...
| REV-07a | BLOCKER ontológico | OPEN | **OriginUnity debe distinguir una configuración real de una mera suma de raíces sin depender del candidato.** | $\operatorname{OriginConfig}_i$ / $\operatorname{UnitFact}_i$. | OriginUnity queda definida por UnitFacts independientes que cruzan toda partición no trivial del seed. UnitFact no puede mencionar el contenedor/Seed/OriginUnity ni el target. NoUnitFactBridge implica directamente ¬OriginUnity. Falta justificar qué relaciones concretas satisfacen UF1–UF5. |
