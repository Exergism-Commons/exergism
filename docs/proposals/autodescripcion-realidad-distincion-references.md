...
- **CPython 3.12 — implementación de `multiprocessing` (`connection.py`, `queues.py`, `popen_spawn_posix.py`, `resource_tracker.py`).** La fuente muestra que en POSIX `Pipe(duplex=True)` usa `socket.socketpair()`, que `Connection` aplica serialización/framing y lee/escribe sobre descriptors, que `Queue` añade buffer/locks/semaphore/feeder thread, y que `spawn` usa bootstrap pipes y el `resource_tracker`. Se usa únicamente para identificar dependencias reales del realizador y falsar una gramática demasiado pobre; no se eleva la implementación CPython a criterio ontológico. https://github.com/python/cpython/tree/3.12/Lib/multiprocessing
