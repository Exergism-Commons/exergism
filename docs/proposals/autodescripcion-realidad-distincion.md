...
8. **interface/memoization/baking:** InterfaceContract/RoleAdequate quedan formalizados; cross-role composition queda condicionalmente resuelta y MemoState/update/invalidation resueltos a nivel de criterio. B1–B10 quedan ahora coordinados: target equivalence se mide por quotient contractual, B7 exige decoder source-class, ProvLink separa lineage de mera reimplementación, product Bake requiere soundness conjunta, composición vertical requiere StageCompatibility y BS-T2/RehydrateSource+RehydrateAdequate gobiernan pérdida/recuperación sin side information mágica. Permanecen ContinuationProfile/TR-M, InterfaceWall fuerte y $\Omega_i$.
...
Memoization no es hashing ni criterio automático de contexto: $\operatorname{MemoIndividuated}\not\Rightarrow\operatorname{ContextIndividuation}$. $\mathcal T_\rho+\mathbb I^\rho$ fijan $\operatorname{Cont}_{i,\rho}$ y $\mathcal K$ solo abrevia la prueba bajo RoleAdequate; CR1–CR8 gobiernan composición cross-role. Para un contract state $\Theta$, MemoSound exige $\ker(\mu)\subseteq E_\Theta$ y refinements pueden exigir rehydration. Bake se tipa por quotients contractuales source/target: B7 requiere $q^S_\Theta(u)=d_\beta(q^T_{\Theta,k}(\sigma))$, por lo que equivalencia target inducida solo puede colapsar diferencias autorizadas. ProvLink es independiente de esa equivalencia y bloquea inferir genealogía desde semejanza funcional. Soundness individual no garantiza product Bake soundness bajo SynRel; una cadena vertical exige que el target intermedio sea SourceUnit admisible y preserve el quotient correcto. Una vez colapsada una diferencia, BS-T2 impide recuperarla por postprocesamiento; RehydrateSource exige que la side information adicional exista, esté grounded y sea accesible. InterfaceWall sigue siendo subdeterminación relativa al canal, no Muro fuerte.
