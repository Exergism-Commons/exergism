...
**Estado: PARTIAL — REV-07.** XR-1 descarga GenSound/GenComplete, GCExists, GeneUnit y scope exacto para una instancia DTS mínima. La realización host/local verifica OR2–OR9 y CI aporta evidence de OR1; WitnessCovariance elimina el observador privilegiado. `NFA-T1` demuestra No-Free-Actuality: si las premisas siguen siendo compatibles con `NoR`, no pueden implicar `ExistsR`. La ruta realizacional queda `RSP + XR1-REAL + IndAdequate_XR1 -> ExistsR`; la ruta a priori exige cerrar NQ. REV-07b/c/d siguen abiertos universalmente pero no bloquean XR-1.
