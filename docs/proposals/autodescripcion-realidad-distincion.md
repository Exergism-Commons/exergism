...
Por tanto, REV-01, REV-02 y REV-04 están cerrados en sus criterios originales, mientras REV-03 permanece PARTIAL por alcance doctrinal general. Sin embargo, su definición event-local se reutiliza ahora sin modificación en XR-ε: EpsilonAlignment EA1–EA8 + TR-M permiten una descarga emergentista restringida de IA0 mediante $\varepsilon$RSP.
...
| REV-03 | PARTIAL general / REUSED en ExistsR | Existe una definición de emergencia event-local, independiente de $R$, basada en macro-invariancia, OrgWitness y capacidad dinámica habilitada. XR-$\varepsilon$ reutiliza exactamente esa definición: una emergencia efectivamente realizada, OR1–OR9, EA1–EA8 y frontera TR-M alineada proporcionan la ruta restringida $\varepsilon$RSP para IA0. Sigue pendiente validar el alcance doctrinal general de la noción mínima en casos ricos. |
...
| REV-07g/07h | PARTIAL global / major subcriteria stress-tested | ContextIndividuation es objetivo y witness-free; TR-M permite contextos anidados bajo MC1–MC10. RC-T1 + NFP-T1–T4 cierran rival-cut/no-free-promotion a nivel de criterio; DR-T1 muestra portabilidad formal en SW/BIO/CTRL. EMP-SW-REDIS, EMP-BIO-ECOLI y EMP-PHY-NEST añaden evidencia externa concreta y EMP-T1 bloquea su promoción automática a ontología: el cuello común queda en MC6 + MC7 + IA0. Generation/GenSig estratifica genealogía; OR1–OR9 + WitnessCovariance separan realization de evidence. Permanecen NQ, InterfaceWall→Muro fuerte y $\Omega_i$. |
...
4. $\operatorname{ExistsR}$ incondicional: XR-1 descarga por exhaustión finita GeneUnit/GenComplete/GCExists/scope; OR2–OR9 se verifican ejecutablemente sobre un host realizer distinto y CI aporta evidencia de OR1. `NFA-T1` muestra que ninguna teoría compatible con `NoR` puede implicar `ExistsR`: la ruta realizacional requiere una premisa actual positiva, mientras la única ruta a priori restante es NQ. XR1-T queda $\mathrm{RSP}+\mathrm{XR1\text{-}REAL}+\mathrm{IndAdequate}_{XR1}\Rightarrow\operatorname{ExistsR}$;
...
**Estado: PARTIAL — REV-07.** XR-1 descarga GenSound/GenComplete, GCExists, GeneUnit y scope exacto para una instancia DTS mínima; además XR-$\varepsilon$ realiza el toy emergentista original (path→cycle): C1/Macro, OrgWitness, novedad y capacidad `activate` se verifican ejecutablemente junto con OR2–OR9. La ruta específicamente exergista queda $\varepsilon\mathrm{RSP}+\operatorname{RealizedEmergence}_{XR1}+\mathrm{IndAdequate}^{1\text{--}10}_{XR1}\Rightarrow\operatorname{ExistsR}$. `NFA-T1` sigue impidiendo convertirla en prueba a priori: la emergencia realizada contiene actualidad positiva. REV-07b/c/d siguen abiertos universalmente pero no bloquean XR-$\varepsilon$.
