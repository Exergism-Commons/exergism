...
8. **interface/memoization/baking:** InterfaceContract/trace semantics + IC1–IC10 y RoleAdequate RKA1–RKA10 quedan formalizados; $\mathcal K$ es solo base característica de toda $\operatorname{Cont}_{i,\rho}$ y bases adecuadas inducen el mismo kernel. Permanecen composicionalidad cross-role, MemoState ontic/semantic + update/invalidation, B1–B10 general, ContinuationProfile/TR-M, InterfaceWall fuerte y $\Omega_i$.
...
Memoization no es hashing ni criterio automático de contexto: $\operatorname{MemoIndividuated}\not\Rightarrow\operatorname{ContextIndividuation}$. La memo-equivalence ya no se define por una test suite escogida: $\mathcal T_\rho+\mathbb I^\rho$ fijan una especificación de continuación y su menor clausura $\operatorname{Cont}_{i,\rho}$; la equivalencia exige indistinguibilidad bajo todas esas continuaciones. $\mathcal K$ solo puede abreviar la prueba si RoleAdequate/RKA1–RKA10 demuestra characteristic completeness; dos bases adecuadas inducen necesariamente el mismo kernel. Si $q_{\mathbb I^\rho}(u)=[u]_{\equiv_{\mathbb I^\rho}}$, un Bake interface-sound debe admitir $d_\beta$ con $q_{\mathbb I^\rho}=d_\beta\circ Bake$, luego $\ker(Bake)\subseteq\ker(\mathbb I^\rho)$. InterfaceWall sigue siendo subdeterminación relativa al canal, no Muro fuerte.
