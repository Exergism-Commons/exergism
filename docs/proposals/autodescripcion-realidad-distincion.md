...
8. **interface/memoization/baking:** InterfaceContract y RoleAdequate quedan formalizados; cross-role composition queda condicionalmente resuelta por CR1–CR8. MemoState distingue ya SemanticMemo de OnticMemoImpl: MemoSound exige $\ker(\mu)\subseteq E_\Theta$, update es factorización por un kernel congruente e invalidation es pérdida de esa inclusión; información quotientada requiere rehydration. Permanecen B1–B10 general/composite, ContinuationProfile/TR-M, InterfaceWall fuerte y $\Omega_i$.
...
Memoization no es hashing ni criterio automático de contexto: $\operatorname{MemoIndividuated}\not\Rightarrow\operatorname{ContextIndividuation}$. $\mathcal T_\rho+\mathbb I^\rho$ fijan $\operatorname{Cont}_{i,\rho}$ y $\mathcal K$ solo abrevia la prueba bajo RoleAdequate. Cross-role composition requiere CR1–CR8 porque equivalencia marginal no implica equivalencia conjunta. Para un contract state $\Theta$, una representación memo es sound exactamente si $\ker(\mu)\subseteq E_\Theta$; OnticMemoImpl añade actualidad/grounding y update reentrante. Un refinement que rompe esa inclusión invalida el memo; si dos histories ya fueron colapsados, ninguna función del memo viejo puede separarlos de nuevo, por lo que hace falta rehydration desde source/history/provenance adicional. Un Bake interface-sound sigue requiriendo $q_I=d_\beta\circ Bake$ y la soundness individual no garantiza soundness del product Bake. InterfaceWall sigue siendo subdeterminación relativa al canal, no Muro fuerte.
