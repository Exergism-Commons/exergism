...
8. **interface/memoization/baking:** InterfaceContract/RoleAdequate quedan formalizados; cross-role composition queda condicionalmente resuelta; MemoState/update/invalidation, B1–B10 y ContinuationProfile/FaithfulContinuation quedan resueltos a nivel de criterio. ContinuationProfile se deriva de PersistenceSpec/PersistenceBasis + memo-equivalence + quotient dynamics; least closure + CP-T0 impiden bases alternativas arbitrarias, PB3 coverage evita omitir MaintRoles y ProfileMorphism + provenance distingue continuidad de réplica. Permanecen TR-M, InterfaceWall fuerte y $\Omega_i$.
...
Memoization no es hashing ni criterio automático de contexto: $\operatorname{MemoIndividuated}\not\Rightarrow\operatorname{ContextIndividuation}$. $\mathcal T_\rho+\mathbb I^\rho$ fijan $\operatorname{Cont}_{i,\rho}$ y RoleAdequate evita test suites post hoc; CR1–CR8 gobiernan composición cross-role. MemoSound exige $\ker(\mu)\subseteq E_\Theta$, refinements pueden exigir rehydration y Bake usa quotients contractuales source/target con provenance separada. Para persistencia, una PersistenceSpec source-side fija MaintRole/MaintStep/provenance/novelty y PersistenceBasis PB1–PB8 toma su least closure; CP-T0 exige invariancia entre realizaciones equivalentes y PB3 fija el scope completo de una FaithfulContinuation no cualificada; de $E_\Theta$ y su right-congruent quotient dynamics se deriva $\mathsf{CP}_i^x=\langle Q,q,\mathcal G,\Rightarrow,O,\mathfrak P,\mathcal N\rangle$. El child solo necesita realizar el contrato traducido mediante TargetRealizationProfile; PM1–PM8 + provenance dan FaithfulContinuation. Esta maquinaria todavía no implica TR-M/ContextIndividuation. InterfaceWall sigue siendo subdeterminación relativa al canal, no Muro fuerte.
