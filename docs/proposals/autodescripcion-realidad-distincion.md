...
8. **interface/memoization/baking:** InterfaceContract + IC1–IC10 y RoleAdequate RKA1–RKA10 quedan formalizados; cross-role composition queda condicionalmente resuelta por CR1–CR8: en general $\bigcap_aE_a\not\Rightarrow E_\otimes$, pero bajo composición separable $E_\otimes=\bigcap_aE_a$. Permanecen MemoState ontic/semantic + update/invalidation, B1–B10 general/composite, ContinuationProfile/TR-M, InterfaceWall fuerte y $\Omega_i$.
...
Memoization no es hashing ni criterio automático de contexto: $\operatorname{MemoIndividuated}\not\Rightarrow\operatorname{ContextIndividuation}$. $\mathcal T_\rho+\mathbb I^\rho$ fijan una especificación de continuación y su menor clausura $\operatorname{Cont}_{i,\rho}$; $\mathcal K$ solo abrevia la prueba bajo RoleAdequate/RKA1–RKA10. La composición exige una deuda adicional: dos histories pueden ser equivalentes bajo cada rol aislado y distinguibles tras intercalar roles. RoleCompositionAdequate/CR1–CR8 bloquea esa sinergia oculta; solo bajo composición separable se deriva $E_\otimes=\bigcap_aE_a$. Si aparece poder discriminante nuevo debe declararse un interaction role/contract. Un Bake interface-sound sigue requiriendo $q_I=d_\beta\circ Bake$; para varios roles la soundness individual no garantiza soundness del product Bake. InterfaceWall sigue siendo subdeterminación relativa al canal, no Muro fuerte.
