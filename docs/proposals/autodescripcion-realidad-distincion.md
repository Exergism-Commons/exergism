...
| REV-07g/07h | PARTIAL global / contract-and-rival coverage bottleneck | IA0 = IA0-R + IA0-U. XR-2 mantiene UG4, UG8 y UG6-S descargados. La auditoría arquitectónica retira `HostTheoryAdequate` como blocker autónomo: la semántica de qué diferencias upstream importan ya pertenece a InterfaceContract/RoleAdequate, MC4/MC6/MC7, RealizationEnvelope y rival-cut discipline. `H_MP^XR2` queda como gramática diagnóstica ejecutable, no como teoría exhaustiva del runtime. Los mecanismos de CPython descubiertos son probes de realización; solo son ontológicamente relevantes si sobreviven al contrato, crean un hidden bypass, revelan un rol/interacción omitido o soportan un rival cut. UG5 y UG6-H siguen PARTIAL por contract/role/interactions coverage y same-level rival discrimination. Permanecen NQ, InterfaceWall→Muro fuerte y $\Omega_i$. |
