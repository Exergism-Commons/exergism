...
8. **interface/memoization/baking:** justificar SourceUnit e InterfaceContract antes de recontextualizar; formalizar IC1–IC10, RoleAdequate, MemoState/update/invalidation y B1–B10, con $\ker(Bake)\subseteq\ker(I^\rho)$ y sin promover una role-unit a ContextIndividuation.
...
REV-07h refina la hipótesis de trivialización de REV-07e separando SourceUnit, Interface, Memoization, Bake y GenesisTrivialization. Una estructura compleja debe estar primero unitizada; después una $\operatorname{InterfaceContract}^{\mathsf M}_i(u_i;I_i^\rho,\rho,\iota)$ fija qué diferencias upstream pueden hacer diferencia downstream. Memoization conserva a través de la historia el estado suficiente para seguir satisfaciendo esa interfaz; update/invalidation controla cuándo deja de bastar. Baking se generaliza como $\operatorname{Bake}^{\mathsf M}_{i\to k}(u_i\Downarrow\sigma_k;I^\rho,\beta)$ y realiza target-side el contrato. GenesisTrivialization queda como aplicación ontogénica de Bake dentro de ContextGenesis.

Memoization no es hashing ni criterio automático de contexto: $\operatorname{MemoIndividuated}\not\Rightarrow\operatorname{ContextIndividuation}$. Tampoco compartir interfaz implica identidad upstream. ConservativeBake y QuotientBake se mantienen, pero la no-inyectividad solo es sound cuando $\ker(\operatorname{Bake}^{I^\rho,\beta})\subseteq\ker(I^\rho)$ y no existe hidden bypass hacia detalles que la interfaz declara irrelevantes. La no-inyectividad de una interfaz puede producir subdeterminación genealógica relativa al canal —InterfaceWall— sin demostrar todavía el Muro fuerte. $\Omega_i$ queda como posible canonicalización posterior de estructura generativa, memoization e interfaces módulo trivializaciones admisibles, nunca como fundamento retroactivo de IndexAdmission.
