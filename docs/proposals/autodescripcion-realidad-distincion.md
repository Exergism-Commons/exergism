...
8. **interface/memoization/baking:** InterfaceContract/trace semantics + IC1–IC10 quedan formalizados y probados en modelos Mealy, KV stateful y AC; permanecen RoleAdequate, memo-equivalence/update/invalidation, B1–B10 general, ContinuationProfile/TR-M, InterfaceWall fuerte y $\Omega_i$, sin promover una role-unit a ContextIndividuation.
...
Memoization no es hashing ni criterio automático de contexto: $\operatorname{MemoIndividuated}\not\Rightarrow\operatorname{ContextIndividuation}$. Tampoco compartir interfaz implica identidad upstream. Si $q_{\mathbb I^\rho}(u)=[u]_{\equiv_{\mathbb I^\rho}}$, un Bake interface-sound debe admitir un decoder target-side $d_\beta$ con $q_{\mathbb I^\rho}=d_\beta\circ Bake$; por ello $\ker(Bake)\subseteq\ker(\mathbb I^\rho)$. En el modelo Mealy canónico el quotient es exacto y ambos kernels coinciden. InterfaceWall sigue siendo subdeterminación relativa al canal, no todavía Muro fuerte; $\Omega_i$ sigue siendo output canónico candidato, nunca fundamento retroactivo de IndexAdmission.
