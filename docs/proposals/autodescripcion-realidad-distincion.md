...
| REV-07g/07h | PARTIAL global / XR-2 open-boundary evidence | IA0 = IA0-R + IA0-U. CIT cierra UG4 para XR-1; FR1–FR4 cierran una clase rival finita, pero DTS no demuestra RivalClassCompleteness. Se introduce T-IODTS/InterfaceAssignmentIndependent y XR-2 ejecuta en CI un environment process distinto con IPC real, input `close`, output `activate`, receiver externo, negative control y scope generativo exacto. IAI2/IAI4 quedan descargados para XR-2, pero UG4/UG5/UG6 y RCC1–RCC6 siguen parciales: boundary actuality no implica unity sufficiency. Permanecen NQ, InterfaceWall→Muro fuerte y $\Omega_i$. |
