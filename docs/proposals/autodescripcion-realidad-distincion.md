...
Por tanto, REV-01, REV-02 y REV-04 están cerrados en sus criterios originales, mientras REV-03 permanece PARTIAL por alcance doctrinal general. XR-ε reutiliza esa definición sin modificación, pero la auditoría IA0 corrige su alcance: EA1–EA8 + TR-M descargan IA0-R / realized modal support mediante $\varepsilon$RSP-R; IA0-U exige además `UnitGroundAdequate`.
...
**Estado: PARTIAL — REV-07.** XR-1 descarga GenSound/GenComplete, GCExists, GeneUnit, scope exacto y un evento emergente realizado con OR1–OR9. La auditoría adversarial de IA0 retira el salto desde emergencia/realización a ContextIndividuation: $\varepsilon$RSP-R descarga IA0-R, mientras IA0-U exige `UnitGroundAdequate`. La ruta actual queda $\varepsilon\mathrm{RSP\text{-}R}+\operatorname{RealizedEmergence}_{XR1}+\operatorname{UnitGroundAdequate}_{XR1}+\operatorname{IndAdequate}^{1\text{--}10}_{XR1}\Rightarrow\operatorname{ExistsR}$. NFA-T1 sigue separando actualidad de la ruta a priori NQ.
