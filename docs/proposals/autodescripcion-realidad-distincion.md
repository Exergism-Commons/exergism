...
| REV-07g/07h | PARTIAL global / contract-relative host bottleneck | IA0 = IA0-R + IA0-U. XR-2 mantiene UG4, UG8 y UG6-S descargados, pero HOST-X1 refuta la candidatura `H_MP^XR2` como teoría host adecuada: CPython 3.12/spawn contiene dependencias conocidas omitidas (bootstrap/import, resource tracker, serialization/framing, carrier POSIX y, para Queue, feeder/buffer/synchronization). HMP-T1 sigue siendo PASS solo sobre la gramática declarada. HTA-X1 bloquea el cierre absoluto por auditoría finita; la ruta vigente retiene `HostTheoryAdequate` únicamente como juicio relativo a un contrato host independiente `K_H`, con `CarrierTransparent` para separar soporte de realización de constitución de la unidad. `RealizerCoverageAdequate`, UG5 y UG6-H siguen OPEN/PARTIAL hasta construir y atacar `K_XR2`/`H_XR2+`. Permanecen NQ, InterfaceWall→Muro fuerte y $\Omega_i$. |
...
4. $\operatorname{ExistsR}$ incondicional: XR-1 descarga GeneUnit/GenComplete/GCExists/scope y OR1–OR9, pero la auditoría IA0 muestra que eso solo descarga IA0-R. La ruta realizacional exige además $\operatorname{UnitGroundAdequate}_{XR1}$ para IA0-U; NFA-T1 mantiene separada la ruta a priori NQ. XR1-T queda $\mathrm{RSP\text{-}R}+\mathrm{XR1\text{-}REAL}+\mathrm{UnitGroundAdequate}_{XR1}+\mathrm{IndAdequate}_{XR1}\Rightarrow\operatorname{ExistsR}$;
